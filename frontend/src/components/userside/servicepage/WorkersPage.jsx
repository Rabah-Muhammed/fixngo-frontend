          ...
          {workers.length > 0 ? (
            <>
              <motion.h2
                ...
                className="text-4xl font-bold text-center text-indigo-600 mb-4" // Increased font size
              >
                Select a Worker
              </motion.h2>
              <motion.p
                ...
                className="text-2xl text-center text-gray-600 mb-16" // Increased font size and bottom margin
              >
                for {serviceName}
              </motion.p>
              <motion.div
              ...
              >
                {workers.map((worker, index) => (
                  <WorkerCard key={worker.id} worker={worker} index={index} navigate={navigate} />
                ))}
              </motion.div>
            </>
          ) : (
  ...
  const NoWorkersFound = ({ serviceName, navigate }) => (
    <motion.div
      ...
      transition={{ duration: 0.5 }}
      className="text-center p-12 bg-white rounded-lg shadow-xl max-w-2xl mx-auto mt-16" // Added top margin
    >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={() => navigate("/services")} // Changed to use navigate
  ...
  const WorkerCard = ({ worker, index, navigate }) => (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <button
        onClick={() => navigate(`/worker/${worker.id}/slots`)}
